...
export type BlogPost = {
  ...
  date: string; updatedAt?: string; tags: string[]; draft: boolean;
};

export type Project = {
  slug: string; title: string; description: string;
  date: string; tags: string[]; stack: string[];
  repo?: string; live?: string; featured: boolean;
};
...
export function getBlogPosts(): BlogPost[] {
  return ls('blog').map(file => {
    ...
    return {
      ...
      updatedAt: data.updatedAt ? new Date(data.updatedAt as string).toISOString() : undefined,
